...
void beavalloc_dump(unsigned int leaks_only)
{
    ...
    for (curr = block_list, i = 0; curr != NULL; curr = curr->next, i++)
    {
        if (leaks_only == FALSE || (leaks_only == TRUE && curr->free == FALSE))
        {
            fprintf(stderr, "  %u\t\t%9p\t%9p\t%9p\t%9p\t%u\t\t%u\t\t"
                            "%u\t\t%u\t\t%u\t\t%s\t%c\n",
                    i, curr, curr->next, curr->prev, curr->data, (unsigned)((void *)curr - lower_mem_bound), (unsigned)((void *)curr->data - lower_mem_bound), (unsigned)curr->capacity, (unsigned)curr->size, (unsigned)(curr->capacity + sizeof(struct block_list)), curr->free ? "free  " : "in use", curr->free ? '*' : ' ');
    ...
    else
    {
        ...
        fprintf(stderr, "  Used blocks: %u  Free blocks: %u  "
                ...
                used_blocks, free_blocks, lower_mem_bound, upper_mem_bound);
    }
}

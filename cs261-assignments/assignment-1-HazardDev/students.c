...
struct student *deep_copy_student(struct student *student)
{
  ...
  // init_student(student_copy, student->name, student->id, student->gpa); //This segfaults, but the code below doesn't. 
  student_copy->name           = malloc(sizeof(char) * (strlen(student->name) + 1));
  strcpy(student_copy->name, student->name);

  student_copy->id             = student->id;
  student_copy->gpa            = student->gpa;
...
void destroy_student_array(struct student *students, int num_students)
{
  for (int i = 0; i < num_students; i++)
  {
    free_student(&students[i]); //This works because free(students) == free(&students[0])
  }
  free(students);
  students = NULL;
}
...
{
  struct student *student_pointer = students;
  for (int i = 0; i < num_students; i++)
  {
    if(students[i].gpa > student_pointer->gpa) 
      student_pointer = &students[i];
  }
